...
def resnetBlock(inputToLayer, numFilters, kernel_size):
    # RES1 = tf.layers.conv2d(inputs = inputToLayer, padding = "same", filters = numFilters, kernel_size = kernel_size, activation = tf.nn.relu)
    RES1 = CBR(inputToLayer, numFilters, kernel_size)
    RES2 = CBR(RES1, numFilters, kernel_size)
    RES3 = CBR(RES2, numFilters, kernel_size) 
    return RES1 + RES3
