...
def DLT(batch_size, pred_pts_2_):
	# batch_size = 1
	pred_pts_2_ = tf.reshape(pred_pts_2_ , [batch_size,8,1])
	pts_1_ = tf.convert_to_tensor(corners_, dtype=tf.float32)
	pts_1_ = tf.expand_dims(pts_1_,0)
	pts_1_ = tf.expand_dims(pts_1_,2)

	pts_1_tile = tf.tile(pts_1_, [batch_size,1,1])
	
	pred_pts_2_tile = pts_1_tile + pred_pts_2_
...
# output, condition = transformer(img, H, out_size)
